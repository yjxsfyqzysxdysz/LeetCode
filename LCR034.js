...
var isAlienSorted = function (words, order) {
  const wordMap = new Map([[undefined, -1]])
  for (let i = 0, len = order.length; i < len; i++) {
    wordMap.set(order[i], i)
  }
  for (let i = 0, len = words.length - 1; i < len; i++) {
    for (let j = 0, len = Math.min(words[i].length, words[i + 1].length); j <= len; j++) {
      if (wordMap.get(words[i][j]) > wordMap.get(words[i + 1][j])) return false
      if (wordMap.get(words[i][j]) < wordMap.get(words[i + 1][j])) break
    }
  }
  return true
}
